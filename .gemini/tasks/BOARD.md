...
- [x] **Documentação Completa do Sistema com Prints Reais (`MANUAL_DO_SISTEMA.md`)**: Criação do manual completo com 24 capturas de tela reais de alta resolução (`docs/prints/`), cobrindo Dashboard, Lançamentos, Modal de Transações, Contas a Receber/Pagar, Modo Loja PDV Touch, Pedidos, Ordens de Serviço, Clientes, Fornecedores, Agenda (Grade Horária e Escalas de Trabalho), Estoque (Hub de Perecíveis), Relatórios (DRE e Kardex), Notificações, Configurações e Responsividade Mobile.
- [x] **Ajuste de Posição do Suporte Flutuante no Mobile**: Elevação da posição do botão de suporte (`SupportWidget.tsx`) para `bottom-20 md:bottom-6` e `right-4 md:right-6`, evitando sobreposição com o menu de navegação inferior fixo (`Navigation.tsx`) em celulares e telas pequenas, além de ajuste de responsividade no card de chat para evitar transbordamento da tela.
- [x] **Ajuste de Seleção de PIX no PDV (Modo Loja)**: Remoção do modal de QR Code fictício (`isPixModalOpen`) ao clicar em PIX no fechamento do cupom. Agora o botão "PIX" funciona exclusivamente como seleção da forma de pagamento para registro do caixa (assim como Cartão e Dinheiro), sem abrir telas desnecessárias com códigos não integrados.
- [x] **Escala de Trabalho e Horários de Atendimento na Agenda**: Implementação completa da gestão de escalas de trabalho com suporte a múltiplos turnos/intervalos por dia (ex: 08:00 às 12:00 e 14:00 às 18:00 com pausa de almoço automática), tanto a nível geral do estabelecimento quanto individualmente por colaborador. Integração visual na Grade Horária (`ScheduleTimelineView`) destacando expedientes abertos, intervalos de pausa/almoço e horários fechados, aviso amigável no formulário de agendamento (`AppointmentDialog`) contra horários fora da jornada, nova aba de configuração interativa (`ScheduleShiftManager`) com atalho para replicar dias úteis, migration SQL para `collaborator_schedules` e testes unitários dedicados em `ScheduleShifts.test.tsx`.
...
- [x] **Modo Loja & PDV Touch (Frente de Caixa)**: Implementação de frente de caixa touch moderna com catálogo de produtos/serviços, atalhos de teclado (F2, F4, F8, F10), leitor de código de barras, modo quiosque/fullscreen, cupom de atendimento responsivo com auto-scroll e reutilização dos componentes oficiais (`ProductDialog`, `ServiceTypeDialog`). Correção dos payloads de inserção em `transactions` e `order_items` para sanar erros 400 (Bad Request) do Supabase.
...
- [x] **Leitor de Scanner & Geração de PDFs Profissionais em Pedidos e OS**: Integração do Leitor de Scanner de Código de Barras (Leitor Físico USB/Bluetooth e Câmera do Smartphone via Supabase Realtime QR Code) no PDV/Pedidos de Venda e na aba de Peças/Estoque de Ordens de Serviço (com baixa automática de estoque e lançamento no financeiro), além de um mecanismo de geração e estilização de PDF corporativo de alta qualidade com layout comercial (`OrderPdf.ts` e `ServiceOrderPdf.ts`).
- [x] **Modo Loja & Frente de Caixa Touch (PDV Rápido) + Unificação de Modais**: Interface touch-friendly de alta velocidade para comércio e prestadores de serviço com grid de botões grandes, categorias em pílulas, suporte híbrido a produtos e serviços rápidos com 1 clique, atalhos de teclado (F2/F4/F8/F10), calculadora de troco express, modal PIX com QR Code dinâmico, modo tela cheia Kiosk e sincronização total com estoque e financeiro (`StorePos.tsx`). Componentes oficiais `ProductDialog.tsx`, `ServiceTypeDialog.tsx` e `CustomerDialog.tsx` extraídos e unificados para reutilização compartilhada entre Estoque, Agenda, Clientes e Modo Loja sem duplicação de código.
- [x] **Modal de Confirmação ao Finalizar Venda**: Inclusão de modal de confirmação com resumo completo da transação (total, desconto, itens, método de pagamento, cliente, atendente e troco) prevenindo finalizações acidentais no PDV Touch (`StorePos.tsx`).
- [x] **Vinculação de Pedidos no Modal de Lançamentos do Financeiro**: Disponibilização da seleção de Pedidos de Venda no `TransactionDialog` para receitas, com preenchimento e sugestão automática de cliente, valor total, código de referência, descrição e forma de pagamento, persistência bidirecional (`transactions.order_id` e `orders.transaction_id`), bloqueio de alteração em lançamentos existentes e exibição de badges de pedido com ações nas listagens de transações e contas a receber.
- [x] **Modal Completo e Premium de Pedidos de Venda (`OrderDialog` / `OrderReceiptDialog`)**: Upgrade do modal de pedido com abas ricas (Itens, Cliente, Financeiro, Cupom de Impressão), KPI cards estatísticos, layout não-genérico responsivo, cópia de resumo, impressão de comprovante térmico/A4 e geração de PDF comercial.
- [x] **Adaptação Responsiva Completa para Dispositivos Móveis (Smartphones)**: Reformulação e polimento da ergonomia mobile em todas as telas da plataforma (Menu inferior nativo fixo, Drawer/Sheet de módulos, cabeçalhos compactos com seletor flexível de clientes, grids responsivos em Dashboard, cartões touch-friendly em Contas a Pagar/Receber, tabelas com scroll horizontal e colunas protegidas em Pedidos, OS, Estoque, Fornecedores, Agenda e Relatórios, barras de navegação de abas com rolagem horizontal e Cupom Flutuante de checkout rápido no PDV Touch).
- [x] **Leitor de Código de Barras com Câmera Nativa do Dispositivo (`DeviceCameraScanner`)**: Componente reutilizável com mira visual a laser, bipe sonoro via Web Audio API e vibração tátil. Suporte a leitura direta pela câmera do celular e webcam no PDV Touch (`StorePos`), Pedidos (`Orders`), Estoque (`Inventory`) e Ordens de Serviço (`ServiceOrderDialog`), com alternador dinâmico entre Câmera Deste Aparelho e Pareamento com Celular Remoto (QR Code).
